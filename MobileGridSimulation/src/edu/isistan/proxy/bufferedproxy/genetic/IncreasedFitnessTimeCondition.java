...
public class IncreasedFitnessTimeCondition implements TerminationCondition{
	...
	public IncreasedFitnessTimeCondition(Long maxTimeSameFitness, Long maxEvolutionTotalTime, AssignmentFitnessFunction fitnessEvaluator) {
	...
	public boolean satisfiedCondition(ArrayList<Short[]> population){	
		...
		if (System.currentTimeMillis() - startTimeSameFitness > maxTimeSameFitness
			|| System.currentTimeMillis() - startEvolutionTime > maxEvolutionTotalTime) return true;
		
		double currentBestFitness = this.fitnessEvaluator.evaluate(this.fitnessEvaluator.getBestIndividual());
		
		if (lastBestFitness < currentBestFitness){			
			lastBestFitness = currentBestFitness;
			startTimeSameFitness = System.currentTimeMillis();
			System.out.println(startTimeSameFitness+" fitness_found: "+currentBestFitness);			
		}
		return false;
