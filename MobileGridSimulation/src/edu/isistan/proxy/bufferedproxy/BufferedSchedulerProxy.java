...
import edu.isistan.proxy.DataIntensiveScheduler;
...
public abstract class BufferedSchedulerProxy extends DataIntensiveScheduler{
	...
	//when queue threshold equals one, the BufferedScheduler perform an assignment every time a new job arrives,
	//otherwise, it waits until the queue has the number of jobs indicated by this field to start job assignments.
	//setting a value greater than one, allows the scheduler to assign jobs in an order different from the job arrival  
	...
	public BufferedSchedulerProxy(String name, String bufferValue) {
		...
		if (bufferValue.compareTo("") != 0){			
			...
			System.out.println("[WARN] Since no buffer size was provided BufferedScheduler was created with a default buffer size of "+bufferSize);
